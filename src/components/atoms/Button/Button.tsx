...
export interface IButtonProps extends ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: 'primary' | 'secondary' | 'nav' | 'round' | 'link' | 'image'
}
...
function getButtonClasses(variant: IButtonProps['variant']) {
  switch (variant) {
    ...
    case 'round':
      return 'bg-icon text-primary rounded-full py-2 px-2 w-12 h-12 flex items-center justify-center transform transition duration-300 hover:scale-110'
    case 'link':
      return 'text-xl hover:scale-110'
    case 'image':
      return 'bg-white  h-2 text-center w-20 hover:scale-110'
  }
}
